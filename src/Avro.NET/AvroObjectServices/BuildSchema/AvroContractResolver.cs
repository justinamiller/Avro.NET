    ...
    internal class AvroContractResolver
    {
        ...
        internal TypeSerializationInfo ResolveType(Type type, MemberInfo member = null)
        {
                ...
                (type.IsEnum() && !type.GetTypeInfo().GetCustomAttributes(false).OfType<DataContractAttribute>().Any()))
            {
                ...
                {
                    Name = Infrastructure.Extensions.TypeExtensions.StripAvroNonCompatibleCharacters(type.Name),
                    Namespace =Infrastructure.Extensions.TypeExtensions.StripAvroNonCompatibleCharacters(type.Namespace),
                    Nullable = isNullable
                };
            }
            ...
            {
                Name =Infrastructure.Extensions.TypeExtensions.StripAvroNonCompatibleCharacters(dataContract?.Name ?? type.Name),
                Namespace = Infrastructure.Extensions.TypeExtensions.StripAvroNonCompatibleCharacters(dataContract?.Namespace ?? type.Namespace),
                Nullable = isNullable,
